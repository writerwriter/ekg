...
class Transformer(tf.keras.Model):
    def __init__(self, model_dim=64, num_heads=8, num_encoder_layers=1, 
        ...
        self.encoder = TransformerEncoder(model_dim, num_heads, dim_feedforward, dropout, activation, normalize_before, enc_norm, num_encoder_layers, name='encoder')

        dec_norm = LayerNormalization(epsilon=1e-5, name='norm')
        self.decoder = TransformerDecoder(model_dim, num_heads, dim_feedforward, dropout, activation, normalize_before, dec_norm, num_decoder_layers, name='decoder', return_intermediate=return_intermediate_dec)

    def call(self, source, mask, query_encoding, pos_encoding, training=False):
        batch_size, signal_length = [tf.shape(source)[i] for i in range(2)]
        ...
        memory = self.encoder(source, source_key_padding_mask=mask, pos_encoding=pos_encoding, training=training)

        hs = self.decoder(target, memory, memory_key_padding_mask=mask, pos_encoding=pos_encoding, query_encoding=query_encoding, training=training)

        hs = tf.transpose(hs, [0, 2, 1, 3])
        memory = tf.transpose(memory, [1, 0, 2])
        memory = tf.reshape(memory, [batch_size, signal_length, self.model_dim])
...
class TransformerDecoder(tf.keras.Model):
    def __init__(self, model_dim=256, num_heads=8, dim_feedforward=2048,
                 dropout=0.1, activation='relu', normalize_before=False, norm=None,
                 num_decoder_layers=1, return_intermediate=False, **kwargs):
        super().__init__(**kwargs)
