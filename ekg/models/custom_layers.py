...
class FrozenBatchNorm2D(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.weight = self.add_weight(name='weight', shape=[input_shape[-1]], initializer='zeros', trainable=False)
        self.bias = self.add_weight(name='bias', shape=[input_shape[-1]], initializer='zeros', trainable=False)
...
class Linear(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.kernel = self.add_weight(name='kernel', shape=[self.output_dim, input_shape[-1]], initializer='zeros', trainable=True)
